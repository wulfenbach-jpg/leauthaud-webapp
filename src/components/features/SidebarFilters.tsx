  ...
  return (
    <aside className="lg:w-64 space-y-8 shrink-0">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest">Research Criteria</h3>
        <button onClick={onClearFilters} className="text-xs font-bold text-emerald-600 hover:underline">Reset</button>
      </div>
      ...
      <div className="space-y-8 max-h-[calc(100vh-200px)] overflow-y-auto no-scrollbar pr-2">
        {filterCategories.map((cat) => (
          <div key={cat.key} className="border-b border-slate-200 pb-6 last:border-0">
            <h4 className="text-sm font-extrabold text-slate-900 uppercase tracking-wider mb-3">{cat.label}</h4>
                ...
                <label key={opt} className="flex items-center group cursor-pointer">
                  <div className="relative flex items-center">
                    <input
                      ...
                      className="peer w-5 h-5 rounded border-slate-300 text-emerald-600 focus:ring-emerald-500/20 transition-all appearance-none border-2 checked:bg-emerald-600 checked:border-emerald-600"
                    />
                    <svg className="absolute left-0.5 top-1 w-4 h-4 text-white hidden peer-checked:block pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="4" d="M5 13l4 4L19 7" /></svg>
                  </div>
                  <span className={`ml-3 text-sm font-medium leading-none transition-all pt-0.5 ${
                    (filters[cat.key as keyof FilterState] as string[]).includes(opt) ? 'text-emerald-700 font-bold' : 'text-slate-600 group-hover:text-slate-900'
                  }`}>{opt}</span>
                </label>
              ))}
            </div>
